    ...
    def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
        ...
        for idx, img in enumerate(L_paths):
                    ...
                    if seq[i] != seq[-1]:
                        if generate_mode == 'DiffPIR':
                            ...
                            if generate_mode == 'DPS_y0':
                                ...
                                norm_grad, norm = utils_model.grad_and_value(operator=Tx, x=xt, x_hat=xt,
                                                                             measurement=y_t)
            ...
            if calc_LPIPS:
                ...
                logger.info(
                    '{:->4d}--> {:>10s} PSNR: {:.4f}dB LPIPS: {:.4f} ave LPIPS: {:.4f}'.format(idx + 1, img_name + ext,
            ...
            if save_progressive:
                ...
                util.imsave(img_total * 255., os.path.join(E_path,
                                                           img_name + '_sigma_{:.3f}_process_lambda_{:.3f}_{}_psnr_{:.4f}{}'.format(
                                                               noise_level_img, lambda_, current_time, psnr, ext)))
            ...
            if save_LEH:
                ...
                util.imsave(np.concatenate([img_I, img_E, img_H], axis=1),
                            os.path.join(E_path, img_name + '_LEH' + ext))
        ...
        logger.info('------> Average PSNR of ({}), sigma: ({:.3f}): {:.4f} dB'.format(testset_name, noise_level_model,
                                                                                      ave_psnr))

        if calc_LPIPS:
            ave_lpips = sum(test_results['lpips']) / len(test_results['lpips'])
            logger.info('------> Average LPIPS of ({}) sigma: ({:.3f}): {:.4f}'.format(testset_name, noise_level_model,
                                                                                       ave_lpips))
