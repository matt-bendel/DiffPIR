...
def main():
    ...
    logger.info(
        'model_name:{}, sr_mode:{}, image sigma:{:.3f}, model sigma:{:.3f}'.format(model_name, sr_mode, noise_level_img,
                                                                                   noise_level_model))
    logger.info(
        ...
        for k_index in range(k_num):
            ...
            def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
                ...
                for idx, img in enumerate(L_paths):
                    ...
                    rhos, sigmas, sigma_ks = torch.tensor(rhos).to(device), torch.tensor(sigmas).to(
                        device), torch.tensor(sigma_ks).to(device)
                    ...
                    x = sqrt_alphas_cumprod[t_start] * (2 * x - 1) + sqrt_1m_alphas_cumprod[t_start] * torch.randn_like(
                        x)
                    ...
                    for i in range(len(seq)):
                            ...
                            if seq[i] != seq[-1]:
                                if generate_mode == 'DiffPIR':
                                    if sub_1_analytic:
                                        ...
                                        norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=x0,
                                                                                     x_hat=x0, measurement=2 * y - 1)
                                    ...
                                    down_sample = Resizer(x.shape, 1 / sf).to(device)
                                    if generate_mode == 'DPS_y0':
                                        norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=x,
                                                                                     x_hat=x0, measurement=2 * y - 1)
                                        ...
                                        y_t = sqrt_alphas_cumprod[t_i] * (2 * y - 1) + sqrt_1m_alphas_cumprod[
                                            t_i] * torch.randn_like(y)  # add AWGN
                                        # y_t = y_t/2 + 0.5
                                        # norm_grad, norm = utils_model.grad_and_value(operator=down_sample,x=x, x_hat=xt, measurement=y_t)    # no need to use
                                        norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=xt,
                                                                                     x_hat=xt, measurement=y_t)
                                        x = xt - norm_grad * lambda_ * norm / (rhos[t_i]) * 0.35
                                        x = x.detach_()
                                        pass

                            # add noise back to t=i-1
                            if (generate_mode == 'DiffPIR' and model_out_type == 'pred_xstart') and not (
                        ...
                        if (i + 1) in [10, 25, 50]:
                            ...
                            plt.imsave(f'motivation_fig/sr_bicubic4_diffpir_{idx}_{im_count}.png', clear_color(x0[0]))
                            im_count += 1
                        ...
                        if save_progressive and (seq[i] in progress_seq):
                            ...
                            if log_process:
                                logger.info(
                                    '{:>4d}, steps: {:>4d}, np.max(x_show): {:.4f}, np.min(x_show): {:.4f}'.format(
                                        seq[i], t_i, np.max(x_show), np.min(x_show)))
                    ...
                    if calc_LPIPS:
                        ...
                        logger.info(
                            '{:->4d}--> {:>10s} -- sf:{:>1d} --k:{:>2d} PSNR: {:.4f}dB LPIPS: {:.4f} ave LPIPS: {:.4f}'.format(
                                idx + 1, img_name + ext, sf, k_index, psnr, lpips_score,
                                sum(test_results['lpips']) / len(test_results['lpips'])))
                    else:
                        logger.info(
                            '{:->4d}--> {:>10s} -- sf:{:>1d} --k:{:>2d} PSNR: {:.4f}dB'.format(idx + 1, img_name + ext,
                                                                                               sf, k_index, psnr))

                    if save_E:
                        util.imsave(img_E, os.path.join(E_path, 'samples/' + img_name + '_x' + str(sf) + '_k' + str(
                            k_index) + '_' + model_name + ext))
                    ...
                    if save_progressive:
                        ...
                        util.imsave(img_total * 255., os.path.join(E_path,
                    ...
                    if save_LEH:
                        ...
                        util.imsave(np.concatenate([img_I, img_E, img_H], axis=1), os.path.join(E_path,
                                                                                                img_name + '_x' + str(
                                                                                                    sf) + '_k' + str(
                                                                                                    k_index) + '_LEH' + ext))
                    ...
                    if save_L:
                        util.imsave(img_L,
                                    os.path.join(E_path, img_name + '_x' + str(sf) + '_k' + str(k_index) + '_LR' + ext))
                ...
                if calc_LPIPS:
                    ...
                    logger.info(
                        '------> Average LPIPS of ({}) scale factor: ({}), kernel: ({}) sigma: ({:.3f}): {:.4f}'.format(
